...
2. 安全性：只有授權管理員可觸發
3. 可指定要執行的命令（如 `/check-issues`、`/check-actions`、自訂 prompt）
4. 能看到執行結果
...
CREATE TABLE remote_tasks (
  ...
  command TEXT NOT NULL,           -- 要執行的命令，如 '/check-issues'
...
<div class="admin-panel">
  ...
  <div class="flex gap-2">
    <button onclick="triggerTask('/check-issues')">巡檢 Issues</button>
...
# 執行命令
cd ~/2026aipm-projects/2026biz && claude -p "/check-issues" --dangerously-skip-permissions
```
...
# 標題：[urgent] 執行 /check-issues
# 標籤：urgent

# 或用 gh CLI（如果手機有安裝）：
gh issue create -R aipmtw/aipmtw.github.io -t "[urgent] /check-issues" -l urgent
```

修改 `/check-issues` SOP，加入 `urgent` 標籤偵測：在排程的空檔也輪詢 urgent issues。
...
| 授權 | 僅 aipmtw 管理員可觸發 |
| 命令注入 | 白名單限制可執行的命令（僅 `/check-issues`、`/check-actions` 等已知指令） |
